...
The helicopter does not end in the desired point because there's significant clockwise bias when the helicopter is left with a zero pitch input.
...
Feedback $k$ is computed using the following script:

\lstinputlisting[language=Matlab]{compute_lq.m}

This script uses $A1$ and $B1$ which are defined in the PROBLEM2FILE.
